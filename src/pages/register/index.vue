...
import { getCaptcha, getPhoneCode, register } from "@/api/user";
...
if(isSixDigits(invitationCode.value.trim())){
  ...
  await register({
    ...
    zfmm: rsaEncrypt(payPassword.value.trim()) as string,
    userPassword: rsaEncrypt(password.value.trim()) as string,
    deviceModel:'web',
  ...
  <div>
    ...
    <div class="p-4 pt-4 bg-white flex min-h-screen justify-center">
      <div class="max-w-xs w-full">
        <!-- 标题 -->
        <h1 class="text-2xl font-bold mb-9 text-center">注册账户</h1>
        ...
        <van-cell-group class="mt-4 border-b-1 border-b-[#E5E5E5]" :border="false">
          <van-field v-model="password" label-width="10" placeholder="输入密码" :type="showPassword ? 'text' : 'password'"
        ...
        <!-- 支付密码输入 -->
        <van-cell-group class="mt-4 border-b-1 border-b-[#E5E5E5]" :border="false">
          <div style="font-size: 15px; color: #767676;" class="ml-5">设置支付密码</div>
          <van-field
            v-model="payPassword"
            :type="showPayPassword ? 'text' : 'password'"
            label-width="10"
            placeholder="输入支付密码"
            :right-icon="showPayPassword ? 'eye-o' : 'closed-eye'"
            @click-right-icon="showPayPassword = !showPayPassword"
          >
            <template #left-icon>
              <img :src="pwdIcon" class="h-[20px] w-[20px] object-contain" alt="icon" />
            </template>
        ...
        <!-- 输入邀请码 -->
        <!-- <van-cell-group class="mt-4 border-b-1 border-b-[#E5E5E5]" :border="false">
        <van-field v-model="inviteCode" label-width="10" placeholder="输入邀请码" left-icon="lock" />
        </van-cell-group>-->

        <!-- 登录按钮 -->
        <div class="mt-6">
          <van-button class="w-full" round type="primary" @click="handleRegister">注册</van-button>
        </div>

        <!-- 协议勾选 -->
        <div class="text-xs text-gray-500 mt-4 flex items-center">
          <van-checkbox v-model="agreeProtocol" shape="round" icon-size="12" />
          <span class="ml-2" @click="agreeProtocol = !agreeProtocol">
            注册即表示同意
            <a
              target="_blank"
              href="https://www.jingchat.cn/userProtocol.html"
              class="text-blue-600"
              @click.stop
            >《服务条款》</a>和
            <a
