...
init()
function init() {
  getPayMethodListByType('1').then((res) => {
    if (res.code === ResCode.SUCCESS) {
      list.value = res?.data || []
    }
    alipayList.value = list.value.map((item) => {
      return {
        ...item,
        isSelected: false,
      }
...
// 删除逻辑联动
function onDelete() {
  const selectedItems = alipayList.value.filter(item => item.isSelected)
...
<template>
  <div class="font-sans bg-gray-50 flex flex-col h-screen overflow-hidden">
    ...
    <van-nav-bar :border="false" class="custom-nav-bar flex-shrink-0 z-10" @click-left="onClickLeft">
      ...
      <template #title>
        <span class="text-lg text-white font-medium">支付宝列表</span>
      </template>
    </van-nav-bar>
    ...
    <div class="p-4 flex-1 overflow-y-auto">
      <div class="space-y-3">
        <div
          v-for="item in alipayList" :key="item.id"
          class="text-white p-4 rounded-lg bg-[#004A7C] shadow-sm transition-all relative overflow-hidden active:scale-98"
        ...
        >
          ...
          <div class="right-3 top-3 absolute z-20" @click.stop="onToggleSelect(item)">
            ...
            >
              <van-icon v-if="item.isSelected" name="success" color="#004A7C" size="12" />
            </div>
          </div>
    ...
    <div class="px-4 pb-6 pt-2 bg-gray-50 flex-shrink-0">
      ...
      >
        ...
        <span class="text-sm text-gray-700 font-medium">添加支付宝</span>
      </div>
