...
import type { IOfflinePushInfoCreateParams } from '../offlinePushInfoManager/index'
...
export function createOfflinePushInfo(conversation: IConversationModel) {
  const androidInfo = {}
  const apnsInfo = {}
 ...
 */
export async function sendMessages(messageList: ITipTapEditorContent[], currentConversation: IConversationModel, isGroup: boolean) {
...
export function handleMessageWithTyping(cloudCustomData: any) {
  if (!cloudCustomData) {
    cloudCustomData = {}
  }
...
export function sendTyping(inputContentEmpty: boolean, inputBlur: boolean) {
